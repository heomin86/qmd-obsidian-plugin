...
export class BatchEmbeddingProcessor {
        ...
        try {
          for (const result of results) {
            if (result.success && result.embedding) {
              this.insertEmbedding(result.embedding);
              completed++;
            } else {
              failed++;
              if (result.error) {
  ...
  private async processBatchWithRetry(
    chunks: DocumentChunk[],
    maxRetries: number
  ): Promise<Array<{ success: boolean; embedding?: EmbeddingResult; error?: string }>> {
    const results: Array<{ success: boolean; embedding?: EmbeddingResult; error?: string }> = [];
    ...
    for (const chunk of chunks) {
      ...
      results.push({ success, embedding, error: lastError });
    }

    return results;
  }

  private insertEmbedding(result: EmbeddingResult): void {
    const [hash, seqStr] = result.hashSeq.split('_');
    const seq = parseInt(seqStr, 10);

    const embeddingBuffer = new Uint8Array(result.embedding.buffer);

    this.db.run(
      `INSERT INTO content_vectors (hash, seq, pos, embedding)
       VALUES (?, ?, 0, ?)
       ON CONFLICT(hash, seq) DO UPDATE SET embedding = excluded.embedding`,
      [hash, seq, embeddingBuffer]
    );
    ...
    this.db.run(
      ...
      [result.hashSeq, JSON.stringify(embeddingArray)]
    );
  }
